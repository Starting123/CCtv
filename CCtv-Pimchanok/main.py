...
camera = cv.VideoCapture(0)      #จำนวนกล้องที่มี


url = 'https://notify-api.line.me/api/notify'     # Url ของ Line และ token
token = 'YyC8BVO6omqWQ9SaE3O3SsTehmQvWBKxdC0mLrrkquB'

# กำหนดหัวข้อเสริม (headers) ไปยัง LINE Notify
headers = {
...
msg = 'ตรวจพบความเคลื่อนไหว'  #กำหนดข้อความ


while camera.isOpened():    #คำสั่งเปิดกล้อง
    ...
    # ทำการบลอร์ภาพสีเทา
    blur = cv.GaussianBlur(gray, (8, 8), 0)
   ...
   # cv.drawContours(screen1, contours , -1,(0,255,0),2)
    ...
    for movement in contours:                   #คำสั่งตรวจจับตลอดเวลา
        if cv.contourArea(movement) < 8000:     # ตรวจจับความเคลื่อนไหว
            continue
        x, y, height, width = cv.boundingRect(movement)
        cv.rectangle(screen1, (x, y), (x+height, y+width), (0, 288, 0), 4)    

        notify = requests.post(url, headers=headers, data={'message': msg})
        print(notify.text)
        
    # รอรับข้อมูลและทำการแสดงภาพบนหน้าต่างที่แสดงบน "pyCCTV"
    if cv.waitKey(10) == ord('q'):    # คำสั่งปิดกล้อง
        break
    
    cv.imshow('pyCCTV', screen1)
    
    #สิ้นสุดคำสั่ง และคำสั่งจะลูปไปเรื่อยๆจนกว่าจะทำการปิดกล้อง
